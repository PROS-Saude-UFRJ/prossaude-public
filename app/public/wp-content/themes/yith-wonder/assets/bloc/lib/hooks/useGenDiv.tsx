...
import {
  GenDivProps,
  UseGenDivReturn,
} from "../global/declarations/interfacesCons";
import { handleGenRender } from "../locals/edFisNutPage/edFisNutReactHandlers";
import { person } from "../../vars";
export default function useGenDiv({
  onSetGen,
  genValueRef,
}: GenDivProps): UseGenDivReturn {
  ...
  useEffect(() => {
    (gr.current ??= document.getElementById("genId") as HTMLSelectElement),
      (gbr.current ??= document.getElementById(
        "genBirthRelId"
      ) as HTMLSelectElement),
      (gtr.current ??= document.getElementById(
        "genTransId"
      ) as HTMLSelectElement),
      (gar.current ??= document.getElementById(
        "genFisAlinId"
      ) as HTMLSelectElement);
    try {
      ...
      if (
        ...
        const g =
            gr.current ??
            (document.getElementById("genId") as HTMLSelectElement),
          gb =
            gbr.current ??
  ...
  useEffect(() => {
    const handleResize = (): void => {
      if (
        !(
          gbr.current instanceof HTMLElement &&
          gtr.current instanceof HTMLElement
        )
      )
        return;
  ...
  useEffect(() => {
    const handleResize = (): void => {
      if (
        !(
          gar.current instanceof HTMLElement &&
          gr.current instanceof HTMLElement
        )
      )
        return;
    ...
    try {
      ...
      if (
        !r.current?.dataset.equalizing ||
        r.current?.dataset.equalizing !== "true"
      ) {
        addEventListener("resize", handleResize);
