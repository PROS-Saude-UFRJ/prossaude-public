...
export function handleGenRender({
  ...
  person.dispatchGen(
    fluxGen({ g, gb, gt, ga }, setGenTrans, setGenBirthRel, setGenFisAlin)
  );
}
export function handleQueryForRefs(
  ...refs: {
    ...
    fallback?: { p?: string; selector: string };
  }[]
): void {
  for (const { id, r, fallback } of refs) {
    if (r) r.current = document.getElementById(id);
    if (!r.current && fallback?.p)
      r.current = document
        .getElementById(fallback.p)
        ?.querySelector(fallback.selector);
  }
}
