...
import {
  ActiveTargInps,
  TargInps,
} from "../../../lib/global/declarations/interfaces";
import {
  NafTypeValue,
  Protocol,
...
export function addRowAtivFis(
  ...
  if (typeof context === "string" && tBodyContainer) {
    ...
    newRow.innerHTML = `
    <td class="tabCelAtFis tabCelAtFis${context}" id="tabCelRowAtFis${context}${count}_1">${
      count - 1
    }&#41</td>
    <td class="tabCelAtFis tabCelAtFis${context} tabCelLeft" id="tabCelRowAtFis${context}${count}_2">
      <input type="text" placeholder='Preencha aqui o nome da Atividade Física ${title} ${
      count - 1
    }' 
    ...
    <td class="tabCelAtFis tabCelAtFis${context} tabCelLeft" id="tabCelRowAtFis${context}${count}_3">
      <input type="number" 
      placeholder='Preencha aqui o Número de Semanas para a Atividade Física ${title} ${
      count - 1
    }'
    ...
    <td class="tabCelAtFis tabCelAtFis${context}" id="tabCelRowAtFis${context}${count}_4">
      <input type="number" 
      placeholder='Preencha aqui o Tempo de Sessão da Atividade Física ${title} ${
      count - 1
    }'
    ...
    data-xls='Tempo de Sessão Mínimo para Atividade Física ${title} ${
      count - 1
    }'
    required />
    </td>
    <td class="tabCelAtFis tabCelAtFis${context} tabCelRight" id="tabCelRowAtFis${context}${count}_5">
      <input type="number" 
      placeholder='Preencha aqui o Número de Meses para a Atividade Física ${title} ${
      count - 1
    }'
      min-length="1" 
...
export function switchAutoFill(autoFillBtn: targEl): void {
  try {
    if (
      !(
        ...
        (autoFillBtn instanceof HTMLInputElement &&
          (autoFillBtn.type === "checkbox" ||
            autoFillBtn.type === "radio" ||
    ...
    if (
      !(
        locks.current &&
        Object.values(locks.current).every((r) => r?.isConnected)
      )
    )
    ...
    const filteredLocks = Array.from(locks.current).filter(
      (lockTabInd) => lockTabInd instanceof HTMLElement
    );
    ...
    filteredLocks.forEach((lock) => {
      try {
        ...
        if (!(lock instanceof HTMLElement))
          throw new Error(`Failed to validate lock instance`);
        if (tabProps.isAutoFillActive) {
          ...
          else if (siblingInput instanceof HTMLSelectElement)
            siblingInput.disabled = true;
          if (
            siblingButton instanceof HTMLButtonElement ||
            siblingButton instanceof HTMLInputElement
          )
          ...
          if (
            !(
              tabProps.gl instanceof HTMLSelectElement ||
              tabProps.gl instanceof HTMLInputElement
            )
          )
            return;
          if (siblingInput instanceof HTMLInputElement)
...
export function getNumCol(evEl: targEl): void {
  try {
    if (!(evEl instanceof HTMLElement))
      throw new Error(`Failed to validate instance of Targeted Element`);
    if (evEl.dataset.col && evEl.dataset.col !== "")
      tabProps.numCol = parseNotNaN(evEl.dataset.col, 2, "int");
    else if (
      (evEl && evEl.id?.match(/[0-9]+_[0-9]+$/g)) ||
      (evEl instanceof HTMLInputElement &&
        evEl.name?.match(/[0-9]+_[0-9]+$/g)) ||
      (evEl instanceof HTMLLabelElement &&
        evEl.htmlFor?.match(/[0-9]+_[0-9]+$/g))
    )
...
export function validateEvResultNum(
  ...
  try {
    ...
    if (!(typeof prop === "number" || typeof prop === "string"))
      throw new Error(`Invalid typeof prop`);
...
export function matchPersonPropertiesWH(): void {
  try {
    ...
    if (
      ...
      person.height = 0;
    if (tiw instanceof HTMLInputElement)
      person.dispatchWeight(validateEvResultNum(tiw, person.weight));
    if (tih instanceof HTMLInputElement)
...
export function updateIndexesContexts(): void {
  try {
    ...
    if (!(person instanceof Person))
      throw new Error(`Failed to validate person instance`);
    if (!(gl instanceof HTMLSelectElement || gl instanceof HTMLInputElement))
      throw new Error(
        `Failed to validate instance of Element for Body Fat Level`
      );
    if (!(fct instanceof HTMLElement))
      throw new Error(`Failed to validate instance of Element for Body Type`);
    evalFactorAtleta();
    ...
    if (
      !(
        tabProps.TMB &&
        tabProps.TMB >= 0 &&
        (tabProps.factorAtvLvl as number) >= 0
      )
    )
      return;
  ...
  try {
    ...
    if (naf.value === "2.2") {
      ...
      if (IMC >= 0 && IMC < 25.0) {
        ...
        throw new Error(`Error obtaining IMC value in fluxFormIMC(), line ${extLine(
          new Error()
        )}.
...
export function updatePGC(ctx: string = "cons"): void {
  try {
    if (!(person instanceof Person))
      throw new Error(`Failed to validate instance of person`);
    ...
    if (
      (tidc instanceof HTMLInputElement &&
        (tidc.type === "number" || tidc.type === "text")) ||
    ...
    if (
      (tipgc instanceof HTMLInputElement &&
        (tipgc.type === "number" || tipgc.type === "text")) ||
      tipgc instanceof HTMLSelectElement
    ) {
      tabProps.PGC = parseNotNaN(person.calcPGC(person).pgc.toFixed(4)) ?? 0;
      evalPGCDecay(tipgc)
        ? assignFormatedValue(tipgc, tabProps.PGC)
        : assignFormatedValue(tipgc, tabProps.PGC, 2);
      tabProps.MLG ??= 0;
      ...
      if (
        !(
          tabProps.timlg instanceof HTMLInputElement &&
          tabProps.timlg.type === "number"
        )
      )
        return;
  ...
  try {
    if (
      !(
        ...
        tabProps.naf instanceof HTMLInputElement
      )
    )
      return;
    if (
      !(
        tabProps.sa instanceof HTMLSelectElement ||
        tabProps.sa instanceof HTMLInputElement
      )
    )
      return;
    const nafIntensity =
      tabProps.naf instanceof HTMLSelectElement
        ? tabProps.naf.dataset.intensity ||
          ...
          tabProps.naf.value
        : tabProps.naf.innerText
            .toLowerCase()
            .replace("á", "")
            .replace("intenso", "Intenso") || tabProps.naf.value;
    evalFactorAtvLvl();
    evalActivityLvl();
    caller === "naf"
      ? (tabProps.sa.value = nafIntensity)
      : (tabProps.naf.dataset.intensity = tabProps.sa.value);
  } catch (e) {
    return;
  }
}
export function defineTargInps({
  ...
  try {
    if (!(el instanceof HTMLElement))
      throw new Error(`Error validating instance of element`);
    if (!(parent instanceof HTMLElement))
      throw new Error(`Error validating instance of ancestral`);
    if (typeof ctx !== "string")
      throw new Error(`Failed to validate typeof ctx value`);
    switch (ctx) {
      case "cons": {
        ...
        if (refs) {
          console.log(
            Object.values(refs).map((col: { [k: string]: NlMRef<nlInp> }) =>
              Object.values(col).every(
                (t: NlMRef<nlInp>) => t?.current instanceof HTMLElement
              )
            )
          );
          console.log(
          ...
          switch (num) {
            case 2:
              return Object.fromEntries(
                Object.entries(refs.secondCol).map(([k, v]) => [
                  k.replace(/[0-9]/g, ""),
                  (v as any)?.current,
                ])
              ) as any;
            case 3:
              return Object.fromEntries(
                Object.entries(refs.thirdCol).map(([k, v]) => [
                  k.replace(/[0-9]/g, ""),
                  (v as any)?.current,
                ])
              ) as any;
            default:
              return Object.fromEntries(
                Object.entries(refs.firstCol).map(([k, v]) => [
                  k.replace(/[0-9]/g, ""),
                  (v as any)?.current,
                ])
              ) as any;
      ...
      case "col": {
        let num =
          el.dataset.col && el.dataset.col !== ""
            ...
            : /_/g.test(el.id)
            ? parseInt(
                el.id.slice(el.id.lastIndexOf("_") + 1).replace(/[^0-9]/g, ""),
                10
              )
            : 1;
...
export function switchRequiredCols({
  ...
  try {
    if (!(snc instanceof HTMLSelectElement || snc instanceof HTMLInputElement))
      throw new Error(
        `Failed to validate instance of Number of Appointment Selector`
      );
    if (!tabProps.fsp || !tabProps.fsp.isConnected)
      tabProps.fsp = document.getElementById("fsProgConsId");
    const fsp = tabProps.fsp;
    if (!(fsp instanceof HTMLElement))
      throw new Error(`Failed to validate instance of Fieldset for Tabs`);
    if (!(td instanceof HTMLElement))
      throw new Error(`Failed to validate instance of Table for Skin Folds`);
    if (!(tsv instanceof HTMLElement))
      throw new Error(`Failed to validate instance of Table for Vital Signs`);
    if (!(tma instanceof HTMLElement))
      throw new Error(
        `Failed to validate Table for Anthropometric measurements`
      );
    if (!(tip instanceof HTMLElement))
      throw new Error(`Failed to validate instance of Table for Indexes`);
    if (!areNumConsOpsValid)
      throw new Error(`Invalidated Number of Appointments Options`);
    const numCons = tabProps.numCons;
    if (typeof numCons !== "number" || numCons <= 0 || numCons > 3)
      throw new Error(
        `Invalidated Number for Appointment: Obtained value as ${
          numCons ?? "undefined"
        }`
      );
    ...
    if (
      !(
        fspElements.tabs &&
        Object.values(fspElements.tabs).every((r) => r?.isConnected)
      )
    )
      fspElements.tabs = fsp.getElementsByTagName("table");
    const tabs = fspElements.tabs;
    if (tabs.length === 0)
      throw new Error(`No table was found in the fieldset`);
    if (
      !(
        fspElements.trs &&
        Object.values(fspElements.trs).every((r) => r?.isConnected)
      )
    )
      fspElements.trs = fsp.getElementsByTagName("tr");
    const trs = fspElements.trs;
    if (trs.length === 0)
      throw new Error(`No table row was found in the fieldset.`);
    if (
      !(
        fspElements.cols &&
        Object.values(fspElements.cols).every((r) => r?.isConnected)
      )
    )
      fspElements.cols = fsp.getElementsByTagName("col");
    const cols = fspElements.cols;
    if (cols.length === 0)
      throw new Error(`No table col was found in the fieldset`);
    ...
    if (
      ...
      throw new Error(
        `Failed to validate number of required inputs for tables`
      );
...
export function defineAxes(tab: targEl): number {
  try {
    if (!(tab instanceof HTMLTableElement))
      throw new Error(`Failed to validate Table instance`);
...
export function validateTabInpList(
  ...
  try {
    if (!("length" in validateTabInpList))
      throw new Error(`Passed object is not a List`);
...
export function filterCellsPattern({
  ...
  try {
    ...
    if (!(pattern instanceof RegExp))
      throw new Error(`Failed to validate instance of Pattern argument`);
    if (attr !== "id" && attr !== "name")
      throw new Error(`Failed to validate typeof attr argument`);
    ...
    switch (attr) {
      case "id":
        ...
        break;
      case "name":
        filterInpCell = Array.from(inps).filter((inp) =>
          pattern.test((inp as HTMLInputElement).name)
        );
        break;
      default:
        break;
    ...
    if (filterInpCell.length === 0)
      throw new Error(`Failed to populate list of filtered elements`);
    ...
    for (const h of ncth.current) {
      ...
      let col = parseNotNaN(
        h.dataset.col &&
          h.dataset.col !== "" &&
          Number.isFinite(
            parseNotNaN(h.dataset.col.replace(/[^0-9]/g, ""), 2, "int")
          )
          ? h.dataset.col.replace(/[^0-9]/g, "")
          : `${
              /_/g.test(h.id)
                ? h.id.slice(h.id.lastIndexOf("_") + 1).replace(/[^0-9]/g, "")
                : 2
            }`,
        2,
        "int"
      ...
      try {
        let col = parseNotNaN(
          inp.dataset.col &&
            inp.dataset.col !== "" &&
            Number.isFinite(
              parseNotNaN(inp.dataset.col.replace(/[^0-9]/g, ""), 2, "int")
            )
            ? inp.dataset.col.replace(/[^0-9]/g, "")
            : `${
                /_/g.test(inp.id)
                  ? inp.id
                      .slice(inp.id.lastIndexOf("_") + 1)
                      .replace(/[^0-9]/g, "")
                  : 2
              }`,
          2,
        ...
        const relHeader = inp
          .closest("form")
          ?.querySelector(`.numConsTextHeadCell[data-col="${col}"]`);
...
export function handleSumClick(
  ...
  try {
    if (!(prt instanceof HTMLSelectElement || prt instanceof HTMLInputElement))
      return;
    if (!(td instanceof HTMLElement))
      throw new Error(`Failed to validate Table of Skin Folds reference`);
    if (typeof person !== "object" || !("sumDCut" in person)) return;
    if (
      !(
        rowsDcut.current &&
        Object.values(rowsDcut.current).every((r) => r?.isConnected)
      )
    )
      rowsDcut.current = td.getElementsByClassName("tabRowDCutMed");
    const rowsDCArray = Array.from(rowsDcut.current).filter(
      (rowDC) => rowDC instanceof HTMLTableRowElement
    );
    if (rowsDCArray.length === 0)
      throw new Error(`Failed to populate rowsDCArray`);
    if (!(ev.currentTarget instanceof HTMLElement))
      throw new Error(`Failed to validate event target instance`);
    person.dispatchDC(
      createArraysRels({
...
export function createArraysRels({
  ...
  try {
    ...
    if (typeof btnId !== "string")
      throw new Error(`Failed to validate typeof Button Id as argumented`);
    if (!(btnId?.match(/(?<=_)[0-9]+/) && btnId?.match(/[0-9]+(?=_)/)))
      throw new Error(`Id pattern invalid`);
    if (typeof protocolValue !== "string")
      throw new Error(`Failed to validate typeof Protocol Value`);
    ...
    const btnCol =
      ...
      inpsIds = targColInps.map((inp) =>
        inp ? (inp as HTMLInputElement).id : null
      );
    ...
    for (let iC = 0; iC < arrayRows.length; iC++) {
      if (
        !(
          arrayRows[iC] instanceof HTMLElement &&
          !arrayRows[iC].hasAttribute("hidden")
        )
      )
        continue;
...
export function handleIndEv(
  ...
  try {
    ...
    evalFactorAtleta();
    if (tabProps.isAutoFillActive) {
      const { tiw, tih, tidc, tiimc, timlg, titmb, tiget, tipgc } =
        defineTargInps({
          el,
          parent: fsp,
          refs: refs ?? undefined,
...
export function exeAutoFill(
  ...
  try {
    ...
    if (typeof context !== "string")
      throw new Error(`Failed to validate typeof context argument`);
...
export function runAutoFill(
  ...
  const numRef =
    ctx === "cons"
      ...
      ? parseNotNaN(el.dataset.col || "2", 2, "int") ||
        (/_/g.test(el.id)
          ? parseNotNaN(
              el.id.slice(el.id.lastIndexOf("_") + 1) || "2",
              2,
              "int"
            )
  ...
  if (
    !(
      ...
      Number.isFinite(person.sumDCut)
    )
  )
    person.sumDCut = 0;
  if (tidc instanceof HTMLInputElement)
    ...
    (() => {
      try {
        if (typeof idf !== "string")
          throw new Error(`Failed to validate target string`);
        gl ??=
          tabProps.gl ??
          document.getElementById("gordCorpLvl") ??
          document.querySelector('[data-title*="Gordura Corporal"]');
        if (
          !(gl instanceof HTMLSelectElement || gl instanceof HTMLInputElement)
        )
          return;
        fct ??=
          tabProps.fct ??
          ...
          document.querySelector('[data-title*="Fórmula"]');
        if (
          !(fct instanceof HTMLSelectElement || fct instanceof HTMLInputElement)
        )
          return;
        naf ??=
          tabProps.naf ??
        ...
        if (
          !(naf instanceof HTMLSelectElement || naf instanceof HTMLInputElement)
        )
          return;
        ...
        if (naf.value === "2.2") {
          ...
          if (
            /bi-lock/gi.test(lockGl.innerHTML) &&
            !tabProps.isAutoFillActive
          ) {
...
export function handleCallbackWHS(inpWHS: targEl): void {
  try {
    ...
    for (const i of ["inpWeight", "inpHeight", "tabInpProgSumDCut"])
      for (const t of document.getElementsByClassName(i))
        if (t instanceof HTMLElement)
          t.dataset.target
            ? (t.dataset["target"] = "false")
            : t.setAttribute("data-target", "false");
    ...
    [
      ...
      if (
        inpWHS.classList.contains(
          `inp${p.charAt(0).toUpperCase()}${p.slice(1).toLowerCase()}`
        )
      ) {
        if (!(p in person)) return;
        (person as any)[p] = validateEvResultNum(
          inpWHS,
          parseFloat(inpWHS.value || "0")
        );
