...
export const assignFormatedValue = (
  targ: entryEl,
  numValue: number | string = "0",
  fix: number = 4
): void => {
  const parsedValue = parseNotNaN(
    typeof numValue === "string" ? numValue : numValue.toString(),
    0,
    "float",
    fix
  );
  if (targ.type === "number") {
    ...
    targ.value = new Intl.NumberFormat("pt-BR", {
    ...
    allTabledInps.forEach((tabInp) => {
      ...
      if (!tabInp.dataset.isreal || tabInp.dataset.isreal !== "true") {
        tabInp.addEventListener("input", (): void => {
          if (
            parseFloat(tabInp.value) < 0 ||
            !Number.isFinite(parseFloat(tabInp.value))
          )
            tabInp.value = "0";
        });
