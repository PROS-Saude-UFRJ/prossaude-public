import { looseNum, nlDsb, queryableNode, rMouseEvent } from "./types";
...
export class Person {
  ...
  public calcIMC(personInfo: Person | { weight: number; height: number }): {
    ...
    try {
      if (
        !(
          personInfo instanceof Person ||
          (typeof personInfo.weight !== "number" &&
            typeof personInfo.height !== "number")
        )
      )
    ...
    try {
      ...
      if (atv === "muitoIntenso" && (fa === "mlg" || fa === "peso")) {
        if (fa === "mlg") {
          ...
          if ("weight" in person && w >= 0)
            return { l: "tinsley", v: 24.8 * w + 10 };
          else
            throw new Error(`Error validating weight.
            Obtained value: ${w ?? 0}`);
        } else
          throw new Error(
            `Failed to validate Factor for Athletes.\nObtained value: ${
              fa || "falsish"
            }`
          );
      } else if (
          ...
          case IMC < 25.0 && IMC >= 0:
            switch (g) {
              case "masculino":
                return {
                  l: "harrisBenedict",
                  v: 66 + (13.8 * w + 5.0 * h - 6.8 * a),
                };
              case "feminino":
                return {
                  l: "harrisBenedict",
                  v: 655 + (9.6 * w + 1.9 * h - 4.7 * a),
                };
              case "neutro":
                return {
                  l: "harrisBenedict",
                  v: 360.5 + (11.7 * w + 3.45 * h - 5.75 * a),
                };
              default:
                throw new Error(
          ...
          case IMC >= 25.0:
            switch (g) {
              case "masculino":
                return {
                  l: "mifflinStJeor",
                  v: 10 * w + 6.25 * h - 5.0 * a + 5,
                };
              case "feminino":
                return {
                  l: "mifflinStJeor",
                  v: 10 * w + 6.25 * h - 5.0 * a - 161,
                };
              case "neutro":
                return {
                  l: "mifflinStJeor",
                  v: 10 * w + 6.25 * h - 5.0 * a - 78,
                };
              default:
                throw new Error(
                  `Error validating instance of Person. Obtained instance: ${
          ...
          default:
            throw new Error(
              `Error validating IMC. IMC obtained: ${
                IMC ?? 0
              }; Valor deve ser numérico, positivo e float`
            );
        }
      } else
        throw new Error(
          `Failed to validate Physical Activity level of person:\nObtained value: ${fa}`
        );
    } catch (e) {
      return {
        l: (() =>
          tabProps.fct instanceof HTMLSelectElement ||
          tabProps.fct instanceof HTMLInputElement
  ...
  public dispatchAtvLvl(a: string): void {
    person.atvLvl = ((a: string): a is Intensity =>
      ["sedentario", "leve", "moderado", "intenso", "muitoIntenso"].includes(
        a
      ))(a)
      ? a
      : person.atvLvl || "leve";
...
export class ExportHandler {
  ...
  public handleExportClick(
    ...
    const [message, suspicious] = new ClickEvaluator().evaluateClickMovements(
      ev
    );
    let idf = "";
    if (
    ...
    if (this.exports > 10 || suspicious) {
      suspicious && console.error(message);
    ...
    if (!pw || btoa(pw) !== "cGFzc3dvcmQ=") {
      if (navigatorVars.pt) {
        ...
        console.log(
          "Esta versão de teste de UX usa a seguinte senha: password",
          {
            icon: "🗝",
          }
        );
        ...
        console.log(
          "This UX testing version uses the following password: password",
          { icon: "🗝" }
        );
      }
      return;
    }
    navigatorVars.pt ? alert("Senha validada!") : alert("Password validated!");
    this.#processExportData(context, scope);
  }
  #getTarget(targ: nlDsb, idf: string): nlDsb {
    const el =
      targ ||
      document.getElementById(idf) ||
      document.getElementsByName(idf)[0];
  ...
  #setTimeoutForExport(ev: rMouseEvent, idf: string): void {
    const interv = setInterval(
      () => this.#setCurrTime(this.currTime - 1000),
      1000
    );
    ...
    navigatorVars.pt
      ? console.error(
          `Você está em timeout para exportações. Por favor aguarde ${this.currTime} ou recarregue a página.`
        )
      : console.error(
          `You are in a timeout for exporting. Please wait for ${this.currTime} or reload the page.`
        );
  }
  #processExportData(
    context: string = "undefined",
    ...
    try {
      ...
      alert("A planilha seria exportada clicando aqui!");
  ...
  async #processImages(
    els: (HTMLCanvasElement | HTMLInputElement)[],
    context: string = ""
  ) {
    ...
    for (const el of els) {
      try {
        if (el instanceof HTMLCanvasElement) {
          ...
          canvasBlobs[
            el.id || el.className.replace(/\s/g, "__") || el.tagName
          ] = await res.blob();
...
export class Validator {
  ...
  constructor() {
    this.#validUsers = new Map([
      [
        "YW5kcmVtYWxpbmFAeWFob28uY29tLmJy",
        { p: "YURtTHVGJmluUFNELTA=", a: true },
      ], //am
      [
        "YW5nZWxhZXN0YWdpb0B5YWhvby5jb20uYnI=",
        { p: "b2QmJmVkJiZtZWQz", a: true },
      ], //an
      [
        "QWxhbnNmLm5zYW50YW5hQGdtYWlsLmNvbQ==",
        { p: "dGkmJmFMVkNwczU=", a: true },
      ], //al
      [
        "bnV0cmlhbGluZW1hcnRpbmV6QGdtYWlsLmNvbQ==",
        { p: "bnV0JiZlZGZpczIq", a: true },
      ], //al
      [
        "bWFsaW5lY3Jpc0BnbWFpbC5jb20=",
        { p: "bWwmY3ImJnVGckpQUmE1Kg==", a: true },
      ], //mc
      ["RG9oZDUzNkBnbWFpbC5jb20=", { p: "dGlVQT1hZHNBUyo=", a: true }], //d
      [
        "ZXJpY2hwYWRpbGhhZHNAZ21haWwuY29t",
        { p: "dWFUST1BRFNhbCYmcGQq", a: true },
      ], //e
      [
        "R2Vyc29ubWFjaGFkby5taWJAZ21haWwuY29t",
        { p: "Z3JNaERkJnBzUkE=", a: true },
      ], //g
      ...
      [
        "a2FyZW5iLnNhbnRpYWdvMjdAZ21haWwuY29t",
        { p: "a3JTdEVmJnBzZDdBKg==", a: true },
      ], //k
      [
        "bWFyY296LmFyYXVqb3pAZ21haWwuY29t",
        { p: "bXJTbUJNdGhMcDImR3JwaCoq", a: true },
      ], //ms
      [
        "bWFyaWFjbGFyYWc0MjJAZ21haWwuY29t",
        { p: "bVJjRzQmJnBkMjI0Kg==", a: true },
      ], //mc
      ["ZGV2LnBobWNAZ21haWwuY29t", { p: "cEhwV3BSWU0qJjI=", a: true }], //pd
      ["cGhzYW1wYWlvNTAzQGdtYWlsLmNvbQ==", { p: "cEhTZUYmJnBzMTAq", a: true }], //pe
      [
        "cHJvc3NhdWRldWZyakBnbWFpbC5jb20=",
        { p: "b3JnJiZudXQmJmVkJiZvZCYmbWVkMTkq", a: true },
      ], //p
      ["YXJvbi5iLjk2QGdtYWlsLmNvbQ==", { p: "YXJvbkJhcmJvc2ExMio=", a: true }],
      [
        "cmFmYWVsZWR1YXJkbzIwNDBAZ21haWwuY29t",
        { p: "cmZERCpwc2QtMQ==", a: true },
      ], //r
  ...
  public async checkLogin(u: string, p: string): Promise<boolean> {
    try {
      if (typeof u !== "string") throw new Error(`Failed to validate typeof u`);
      if (typeof p !== "string") throw new Error(`Failed to validate typeof p`);
      const res = await this.#matchUsersMap(u, p);
      if (!res.ok) {
        console.log(`Error ${res.status}: ${res.msg}`, { icon: "❌" });
        console.warn(`Error ${res.status}: ${res.msg}`);
        return false;
      }
      return true;
    } catch (e) {
      console.error(`Error executing checkLogin:\n${(e as Error).message}`);
      return false;
    }
  }
  async #matchUsersMap(
    u: string,
    p: string
  ): Promise<{ msg: string; status: number; ok: boolean }> {
    try {
      const u64 = btoa(u);
      const vU = this.#validUsers.has(u64);
      if (!vU) {
        if (window)
      ...
      const mP = this.#validUsers.get(u64);
      if (!mP) {
        if (window)
          return navigator.language.startsWith("pt-")
            ? {
                msg: "Não há credenciais registradas para o usuário",
                status: 401,
                ok: false,
              }
            : {
                msg: "No credentials registered for the user",
                status: 401,
                ok: false,
              };
        else
      ...
      console.error(`Error executing matchUsersMap:\n${(e as Error).message}`);
      return {
...
export class PacInfoValidator extends PersonValidator {
  ...
  public static validateHistoric(historic: HistoricInfo): boolean {
    ...
    return (
      (historic.type.toLowerCase() === "indefinido" ||
        validTypes.includes(historic.type.toLowerCase() as aptTypes)) &&
