...
import { autoCapitalizeInputs, compProp, parseNotNaN } from "../gModel";
...
export function checkForReset(ev: rMouseEvent): void {
  try {
    ...
    if (res === "CONFIRMAR") {
      ...
      if (!form) {
        navigatorVars.pt
          ? console.error("Não foi possível localizar o formulário")
          : console.error("Failed to locate the formulary");
        return;
      }
      resetForm(form);
    }
...
export async function validateForm(
  ...
  if (form instanceof HTMLFormElement) {
    if (formValidated && form.checkValidity()) form.noValidate = false;
