...
export function checkInnerColGroups(parentEl: targEl): number {
  ...
  try {
    ...
    for (let m = 1; m < validColGroupsChildCount.length; m++)
      (validColGroupsChildCount[m] = validColGroupsChildCount[m - 1]) &&
        pairedColGroupsValid.push(true);
    tabProps.areColGroupsSimilar = pairedColGroupsValid.every(
      (pairedColGroup) => pairedColGroup === true
    )
      ? true
      : false;
  } catch (e) {
  ...
  try {
    ...
    if (
      !(
        ...
        tipgc instanceof HTMLTextAreaElement
      )
    )
    ...
    if (decreasedPGC > tabProps.PGC) {
      ...
      while (decreasedPerson.sumDCut > 0) {
        ...
        if (decreasedPGC < 0 || !Number.isFinite(decreasedPGC))
          decreasedPGC = 0;
      ...
      decreasedPGC <= tabProps.PGC &&
      (tabProps.PGC > 100 || decreasedPerson.sumDCut > 514)
    ) {
      /* eslint-disable */
    ...
    if (person.sumDCut < 0 || !Number.isFinite(person.sumDCut))
      person.sumDCut = 0;
...
export const evalBodyType = (g: string): g is BodyType =>
  ["masculino", "feminino", "neutro"].includes(g);
export function evalFactorAtleta(): boolean {
  if (
    typeof tabProps.factorAtleta !== "string" ||
    tabProps.factorAtleta === ("" as any)
  )
    tabProps.factorAtleta = "peso";
  if (tabProps.factorAtleta !== ("peso" as any))
    tabProps.factorAtleta = tabProps.factorAtleta.toLowerCase() as any;
  if (tabProps.factorAtleta !== ("mlg" as any))
    tabProps.factorAtleta =
      (tabProps.factorAtleta?.toLowerCase() as any) ?? "peso";
  if (tabProps.factorAtleta !== "peso" && tabProps.factorAtleta !== "mlg")
    tabProps.factorAtleta = "peso";
  return (["peso", "mlg"] as FactorAtletaValue[]).includes(
    tabProps.factorAtleta
  );
}
...
export function evalFactorAtvLvl(): boolean {
  if (
    (typeof tabProps.factorAtvLvl !== "string" &&
      typeof tabProps.factorAtvLvl !== "number") ||
    (typeof tabProps.factorAtvLvl === "number" &&
      !Number.isFinite(tabProps.factorAtvLvl))
  )
  ...
  if (![1.2, 1.4, 1.6, 1.9, 2.2].includes(tabProps.factorAtvLvl as number))
    tabProps.factorAtvLvl = 1.4;
  return ([1.2, 1.4, 1.6, 1.9, 2.2] as any).includes(
    tabProps.factorAtvLvl as number
  );
}
export function dispatchFactorAtvLvl(newValue: NafTypeValue): void {
  if (typeof newValue === "string")
    newValue = parseNotNaN(newValue) as NafTypeValue;
  if ([1.2, 1.4, 1.6, 1.9, 2.2].includes(newValue as number))
    tabProps.factorAtvLvl = newValue;
...
export function evalActivityLvl(): boolean {
  if (
    ...
    person.atvLvl = "leve";
  return ["sedentario", "leve", "moderado", "intenso", "muitoIntenso"].includes(
    person.atvLvl
  );
}
export function evalIMC(): boolean {
  if (typeof tabProps.IMC !== "number" || !Number.isFinite(tabProps.IMC))
    tabProps.IMC = 0;
...
export function evalMatchTMBElements(): boolean {
  if (!(tabProps.lockGl instanceof Element))
    tabProps.lockGl = document.getElementById("lockGordCorpLvl");
  if (!(tabProps.spanFa instanceof HTMLElement))
    tabProps.spanFa = document.getElementById("spanFactorAtleta");
  return (
    tabProps.lockGl instanceof Element && tabProps.spanFa instanceof HTMLElement
  );
}
