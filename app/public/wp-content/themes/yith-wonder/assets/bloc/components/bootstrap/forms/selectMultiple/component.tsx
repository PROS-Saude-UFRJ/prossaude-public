  ...
  public render(): JSX.Element {
    return (
      <>
        ...
        <select className="form-select" multiple id={this.id} name={`select_multiple__${this.id}`} data-block={`block__${this.id}`}
        style={{
          width: "90%",
          minHeight: '2.5rem'
        }}
      ...
      if (targ instanceof HTMLSelectElement) updateOpts(targ);
        ...
        >
        <div className="form-text" id={`tip__${this.id}`} contentEditable data-block={`block__${this.id}`}>Escreva uma dica aqui ou remova o texto.</div>
      </>
    );
  }
}
