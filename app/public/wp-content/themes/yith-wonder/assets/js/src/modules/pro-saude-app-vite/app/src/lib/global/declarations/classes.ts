...
import { maxProps, navigatorVars, person, tabProps } from "../../../../src/vars";
import {
  BodyType,
  GordLvl,
  Intensity,
...
} from "../../../../src/lib/locals/edFisNutPage/edFisNutModel";
...
export class Validator {
  ...
  public async checkLogin(u: string, p: string): Promise<boolean> {
    try {
      ...
      if (!res.ok) {
        toast(`Error ${res.status}: ${res.msg}`, { icon: "❌" });
  ...
  async #matchUsersMap(u: string, p: string): Promise<{ msg: string; status: number; ok: boolean }> {
    try {
      const u64 = btoa(u);
