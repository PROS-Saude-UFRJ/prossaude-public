...
import { providers, formData, navigatorVars } from "../../src/vars";
...
import { nlBtn, nlDiv, nlDlg, nlFm, nlInp, validAreas } from "../../src/lib/global/declarations/types";
import {
  addEmailExtension,
  assignFormAttrs,
    ...
    setTimeout(
      () =>
        navigate({
          ...
          search: `${location.search}&new-cons=open`,
        }),
      300,
    );
    return (): void => {
      const searchParams = new URLSearchParams(location.search);
      searchParams.delete("new-cons");
      navigate(
        {
          pathname: location.pathname,
                      ...
                      <ErrorBoundary
                        FallbackComponent={() => (
                          <GenericErrorComponent
                            message={
                              navigatorVars.pt
                                ? `Houve algum erro carregando a lista`
